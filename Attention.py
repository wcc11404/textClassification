...
class Attention(object):
    def __init__(self,mode=2):
        ...
        self.Model_dir = "Attention"  # 模型参数默认保存位置
    ...
    def attention(self,inputs, attention_size):
        ...
        b_omega = tf.get_variable("bias_omega", initializer=tf.random_normal([attention_size], stddev=0.1))
    ...
    def trainModel(self):
        ...
        for epochnum in range(max_epochs):
            ...
            for x_batch, y_batch, batchnum, batchmax in batches:  # 通过迭代器取出batch数据
                ...
                if batchnum % 10001==0 or (epochnum == max_epochs-1 and batchnum == batchmax // 2):
